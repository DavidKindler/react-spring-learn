...
export default class Mmenu {
   ...
   */
  constructor(menu: HTMLElement | string, options?: mmOptions, configs?: mmConfigs) {
  ...
  _initOpened() {
    ...
    let current = lastitem ? lastitem.closest('.mm-panel') : DOM.children(this.node.pnls, '.mm-panel')[0];
   ...
   */
  _getPanelTitle(panel: HTMLElement, dfault?: string | Function): string | object {
    var title: string;
